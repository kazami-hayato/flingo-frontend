          ...
          <div class="practice-item" v-for="(item,index) in examination.questions" :key="index">
            ...
            <el-form-item label="正确答案" label-width="80px" v-show="isEditable">
              <el-select v-model="item.correctAnswer">
                <el-option v-for="option in ['A','B','C','D']" :label="option" :key="option"
...
<script>
  import {getStuExamByParams} from '@/api/system_apis'
  ...
  export default {
    ...
    computed: {
      ...
      score() {
        ...
        return Math.ceil(100*((temp*1.0)/this.examination.totalQuestions))
      },
    },
    methods: {
      /**
      ...
      getExamination() {
        getStuExamByParams(this.listQuery).then(response => {
